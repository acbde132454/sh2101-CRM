...
<body>
	...
	<!-- 阶段状态 -->
	<div style="position: relative; left: 40px; top: -50px;" id="stagesDiv">
		阶段&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<script>
        refresh();
        //第一次显示阶段图标和后面点击修改阶段图标都可以看成刷新阶段图标 arguments
        function refresh(index){
            ...
            $.post("/crm/workbench/transaction/queryStage",{
                ...
                for(var i = 0 ; i < data.length; i++){
                    var stage = data[i];
                    if("绿圈" == stage.type){
                        content += "<span index="+stage.index+" class=\"glyphicon glyphicon-ok-circle mystage\" data-toggle=\"popover\" data-placement=\"bottom\" data-content=\""+stage.text+" "+stage.possibility+"\" style=\"color: #90F790;\"></span>"
                    }else if("锚点" == stage.type){
                        content += "<span index = "+stage.index+" class=\"glyphicon glyphicon-map-marker mystage\" data-toggle=\"popover\" data-placement=\"bottom\" data-content=\""+stage.text+" "+stage.possibility+"\" style=\"color: #90F790;\"></span>";
                    }else if("黑圈" == stage.type){
                        content += "<span index="+stage.index+" class=\"glyphicon glyphicon-record mystage\" data-toggle=\"popover\" data-placement=\"bottom\" data-content=\""+stage.text+" "+stage.possibility+"\"></span>";
                    }else if("黑叉" == stage.type){
                        content += "<span index="+stage.index+" class=\"glyphicon glyphicon-remove mystage\" data-toggle=\"popover\" data-placement=\"bottom\" data-content=\""+stage.text+" "+stage.possibility+"\"></span>";
                    }else if("红叉" == stage.type){
                        content += "<span index="+stage.index+" class=\"glyphicon glyphicon-remove mystage\" data-toggle=\"popover\" data-placement=\"bottom\" data-content=\""+stage.text+" "+stage.possibility+"\" style=\"color: red;\"></span>";
                    }
                    content += "-----------";
                }
                content += "<span class=\"closingDate\">"+new Date().toLocaleDateString()+"</span>";
                $('#stagesDiv').html(content);

                //更新页面的阶段和可能性内容
                //js用于条件判断分类:true/false 1/0 null undefined
                if(index){
                    //取出点击阶段
                    var stage = data[index];
                    $('#stage').text(stage.text);
                    $('#possibility').text(stage.possibility);
                }
                //阶段提示框
                $(".mystage").popover({
                    trigger:'manual',
        ...
        //点击每个图标触发单机事件
        $('#stagesDiv').on('click','.mystage',function () {
           refresh($(this).attr('index'));
        });
